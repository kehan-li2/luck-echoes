...
export async function POST() {
	try {
		...
		const result = await ai.models.generateContent({
			...
			contents: `Give a daily fortune for someone born on ${
				// @ts-expect-error bday
				user.bday.toISOString().split("T")[0]
			}. Include lucky color, lucky stone, a short word of guidance (under 50 words), and ratings (0-100) for Love, Work, and Study.`,
			config: {
				systemInstruction:
